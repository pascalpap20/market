...
const ProductCard = ({ product, handleAddToCart, dropFromCart }) => {
  const { id, nama, gambar, deskripsi, price, currency } = product;
    ...
    <div className="overflow-hidden shadow-lg rounded-lg h-auto w-80 md:w-80 p-4 bg-secondary m-1">
      <div href="#" className="w-full block h-full">
        ...
        <form className="flex flex-row-reverse mt-2" onSubmit={handleSubmit}>
          <button
            className={`${isAddedToCart ? `hidden` : ``} order-4`}
          ...
          <input
            type="text"
            className={`${isAddedToCart ? `hidden` : ``} order-3 w-9 px-1`}
            value={amountNumber}
            onChange={(e) => setAmountNumber(parseInt(e.target.value))}
          />

          <button
            className={`${isAddedToCart ? `hidden` : ``} order-2 mr-1`}
          ...
          {isAddedToCart ? (
            <button
              className="bg-danger rounded px-2 py-1 text-white  hover:bg-primary"
              type="button"
              onClick={handleDrop}
            >
              Drop from cart
            ...
            <button
              className="bg-primary rounded px-2 py-1 text-white  hover:bg-danger"
              type="submit"
            >
              Add to cart
            </button>
