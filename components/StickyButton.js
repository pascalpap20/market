  ...
  return (
    <div>
      <div
        className={`${
          isOpenCart ? `` : `hidden`
        } flex flex-col fixed bottom-0 right-0 mr-8 mb-24 z-20 bg-primary rounded-lg hover:shadow-2xl w-60  overflow-hidden`}
      >
        {itemInCart.map((item) => (
          <div key={item.id}>
        ...
        {itemInCart.length === 0 ? (
          <div className="p-1">No item in cart</div>
        ) : (
          <a
            href={`https://wa.me/${process.env.NO_TELP}?text=${sendMessage}%0D${messageToSend}`}
            target="_blank"
            rel="noreferrer"
            className="p-1 hover:bg-back"
          >
            Send via WhatsApp
          </a>
        )}
